...
from .utils.python import swap_value

# IDA Hooks

class UIHooks(ida_kernwin.UI_Hooks):
    def ready_to_run(ego):
        pass
    def get_lines_rendering_info(ego, out, widget, rin):
        pass
    def populating_widget_popup(ego, widget, popup, ctx):
        pass

class IDPHooks(ida_idp.IDP_Hooks):
    def ev_ending_undo(ego, action_name, is_undo):
        pass

class IDBHooks(ida_idp.IDB_Hooks):
    def auto_empty_finally(ego):
        pass
    def byte_patched(ego, ea, value):
        pass
...
def resolve_symbol(from_ea, name):
    ...
    # XXX: deferred import to avoid breaking .reload() dev helper
...
def remove_ida_actions(popup):
    ...
    class FilterMenu(QtCore.QObject):
        def __init__(ego, qmenu):
            QtCore.QObject.__init__(ego)
            ego.qmenu = qmenu

        def eventFilter(ego, obj, event):
            if event.type() != QtCore.QEvent.Polish:
                return False
            for act in ego.qmenu.actions():
                if act.text() in ["&Font...", "&Synchronize with"]:
                    qmenu.removeAction(act)
            ego.qmenu.removeEventFilter(ego)
            ego.qmenu = None
            return True
